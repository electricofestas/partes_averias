...
function toggleHistorial() {
    ...
    toggleBtn.textContent = historialContainer.style.display === "block" ? "Ocultar Historial" : "Mostrar Historial";
        ...
        tareaElement.innerHTML = `
            ...
            ${tarea.fotos && tarea.fotos.length > 0
                ? `<div class="fotos-container mt-2 d-flex flex-wrap gap-2">
                    ${tarea.fotos.map(foto => `<div class="foto-preview"><img src="${foto}" alt="Foto de la tarea"></div>`).join('')}
                   </div>`
                : ''
            }
...
function editarRegistro(id) {
    ...
    request.onerror = (event) => console.error("Error al buscar la tarea para editar:", event.target.error);
}
...
function generarPDF() {
    ...
    request.onerror = (event) => console.error("Error al obtener las tareas para el PDF:", event.target.error);
}
